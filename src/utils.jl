    ...
    for optm in model.optms
        ...
        if optm.method isa Optim.AbstractOptimizer
            res = DiffEqFlux.sciml_train(lss_f, res.u, optm.method, model.adtype;
                ...
                x_abstol=optm.atol, x_reltol=optm.rtol,
                f_abstol=optm.atol, f_reltol=optm.rtol,
                g_abstol=optm.atol, g_reltol=optm.rtol,
                outer_x_tol=nothing, outer_f_tol=nothing, outer_g_tol=nothing,
                outer_x_abstol=optm.atol, outer_x_reltol=optm.rtol,
                outer_f_abstol=optm.atol, outer_f_reltol=optm.rtol,
                outer_g_abstol=optm.atol, outer_g_reltol=optm.rtol,
                f_calls_limit=0, g_calls_limit=0, h_calls_limit=0,
                allow_f_increases=optm.allow_f_increases, allow_outer_f_increases=optm.allow_f_increases,
                successive_f_tol=1,
